# TYK Solution for TAME 2.0 Module 5.3, Supervised Machine Learning Model Interpretation
...
2. Using casing depth and the feature with the highest variable importance, construct a decision boundary plot. Under what conditions is a well more likely to predict detectable Manganese levels based on a decision boundary plot?
...
```{r}
# Load the data
manganese_data <- data.frame(read_excel("Module5_3_TYKInput.xlsx"))
...
Using the "Module5_2TYKInput.xlsx", use RF to determine if well water data can be accurate predictors of Manganese detection as was done in the previous module. However, this time, incorporate SMOTE in the model. Feel free to use either the `trainControl()` or `createFolds()` function for CV. Extract the variable importance for each predictor on a RF model. What two features have the highest variable importance?
...
for (i in 1:length(manganese_index)){
    ...
    balanced_data_train = smotenc(data_train, "Detect_Concentration")
    ...
    for (j in 1:length(ntree_values)){
        for (k in 1:length(mtry_values)){
            ...
            reg_rf_pred_tune[[k]] = randomForest(Detect_Concentration ~ ., data = balanced_data_train, 
    ...
    reg_rf <- randomForest(Detect_Concentration ~ ., data = balanced_data_train,
...
**Answer**: From the variable importance dataframe and plot, we can see that a sample's location (indicated by longitude and latitude) had the greatest impact on RF followed by pH,stream distance, casing depth, well depth, water sample date, elevation, flow rate, and static water depth. 

<br>

## TYK Question 2

Using casing depth and the feature with the highest variable importance, construct a decision boundary plot. Under what conditions is a well more likely to predict detectable Manganese levels based on a decision boundary plot?
...
highest_predictivity_data = data.frame(manganese_data[,c("Longitude", "Stream_Distance", "Detect_Concentration")])

# Using SMOTE first to balance classes
balanced_highest_predictivity_data = smotenc(highest_predictivity_data, "Detect_Concentration")

# Training RF
rf_detect_manganese = randomForest(Detect_Concentration~., data = balanced_highest_predictivity_data)
```
...
# calling function
grid_df_rf = get_grid_df(rf_detect_manganese, balanced_highest_predictivity_data)
head(grid_df_rf)
```

### Decision Boundary Plot
```{r warning = FALSE, fig.width=7.5, fig.height=6, fig.align='center', out.width = "90%"}
options(repr.plot.width=10, repr.plot.height=7) #changing plot size

ggplot() +
  geom_point(data = manganese_data, aes(x = Stream_Distance, y = Longitude, color = Detect_Concentration),
            position = position_jitter(w = 0.1, h = 0.1), size = 4, alpha = 0.8) + 
  geom_contour(data = grid_df_rf, aes(x = Stream_Distance, y = Longitude, z = as.numeric(Pred_Class == "D")), 
               color = "black", breaks = 0.5) + # adds contour line
  geom_point(data = grid_df_rf, aes(x = Stream_Distance, y = Longitude, color = Pred_Class), 
             size = 0.1) + # shades plot
 
 xlim(0, 1900) + # changes the limits of the x axis
 ylim(-80.84, -80.27) + # changes the limits of the y axis
  ...
  labs(x = 'Stream Distance (ft)') + #changing axis labels

  scale_color_discrete(name = "Arsenic Detection", # renaming the legend
                      labels = c('Non-Detect','Detect')) # renaming the classes
```

**Answer**: There is a significant amount of overlap between detect and non-detect Manganese samples, however wells contaminated with Manganese were more likely to be found further east (longitude > -80.5). It is difficult to make out any meaninful differences in stream distance between detect and non-detect wells.